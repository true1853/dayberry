...
export async function createDealWithEscrow(prisma, input = {}) {
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      // Сопутствующие записи (чат, первое сообщение) создаются в той же
      // транзакции: сделка без чата — это уже расхождение, пусть и не
      // денежное. Внешнего ввода-вывода здесь по-прежнему нет.
      const attached = typeof input.attach === 'function' ? await input.attach(tx, deal) : null;

      return { dealId: deal.id, escrowTransactionId, credits, attached, replayed: false };
    });
  } catch (error) {
