...
// Ключ команды создания привязан к инициатору и его клиентскому id, а не к
// сделке: сделки ещё нет в момент, когда повтор запроса надо отсечь.
export function stableCommandKey(actorId, clientCommandId) {
  return `deal:create:${cleanString(actorId)}:${cleanString(clientCommandId)}`;
}
