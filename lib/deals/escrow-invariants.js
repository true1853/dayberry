...
function summarizeSeverity(buckets, hasDealEscrowLink) {
  const blocking = {};
  const expected = {};
  for (const [name, rows] of Object.entries(buckets)) {
    if (rows.length === 0) continue;
    const isExpected = !hasDealEscrowLink && EXPECTED_BEFORE_LINK_COLUMN.has(name);
    (isExpected ? expected : blocking)[name] = rows.length;
  }
...
export function classifyEscrowCandidateGraph(input = {}) {
  ...
  const severity = summarizeSeverity(buckets, hasDealEscrowLink);
