...
const CAT_KEYWORDS = {
  gadget: ['playstation', 'ps5', 'xbox', 'macbook', 'iphone', 'apple watch', 'ноутбук', 'телефон', 'смартфон', 'наушник', 'монитор', 'консоль', 'планшет', 'гаджет', 'камера', 'фотоаппарат'],
  digital: ['лендинг', 'сайт', 'приложени', 'дизайн', 'реклам', 'таргет', 'курс', 'услуг', 'урок', 'настройк', 'съёмк', 'видео', 'копирайт', 'фотосесси'],
  eco: ['велосипед', 'куртка', 'настол', 'книг', 'одежд', 'кофт', 'сумк', 'кроссовк', 'игруш', 'мебель', 'вещ', 'аксессуар', 'ролик'],
};
...
const WANTS_BY_CAT = {
  gadget: 'Техника Apple, ноутбук, наушники, игровая консоль',
  digital: 'Услуги дизайна, реклама, техника Apple, фотоаппарат',
  eco: 'Книги, настолки, одежда, техника',
};

function detectCat(title, kind) {
  if (kind === 'service') return 'digital';
...
function fallbackDraft(input) {
  ...
  let base = hintPrice(title) || value;
  if (!base || base <= 0) base = 0;
  const num = Math.round(Number(base));
  const isService = kind === 'service' || cat === 'digital';
  ...
  return {
    ...
    reasoning: 'Ориентировочная оценка по похожим объявлениям (внешний ИИ не подключён — добавьте AI_API_KEY для точной оценки).',
    ai: false,
  };
}
...
const SYSTEM_PROMPT = `Ты — эксперт бартерной платформы «Дай бери» в России. Помогаешь пользователю быстро создать качественное объявление обмена без денег (1 балл = 1 рубль).
...
Правила:
- cat строго из списка; kind=service → cat=digital.
- value, aiLow, aiHigh — целые числа > 0. Всегда давай числовую оценку: по данным пользователя, а если их нет — по средней рыночной цене такого товара/услуги в России. aiLow < value < aiHigh.
...
async function callLLM(messages) {
  ...
  if (!content) throw new Error('AI API: пустой ответ');
  return content;
}

function sanitizeDraft(d, input, fallback) {
  const value = Math.max(0, Math.round(Number(d.value) || fallback.value));
  const cat = ['gadget', 'digital', 'eco'].includes(d.cat) ? d.cat : fallback.cat;
  ...
  return {
    ...
    aiLow,
    aiHigh,
    reasoning: (d.reasoning || fallback.reasoning || '').trim().slice(0, 240),
    ai: true,
  };
}
...
const STOPWORDS = new Set([
  ...
  'готов', 'готова', 'готовы', 'обмен', 'обмена', 'обменяю', 'обменя', 'могу', 'есть', 'нужно', 'надо',
  'буду', 'будут', 'ищу', 'меняю', 'отдам', 'отдаю', 'меняться', 'меняюсь', 'вещи', 'вещь', 'товар',
  'услуг', 'услугу', 'услуги', 'техника', 'технику', 'техники', 'и', 'в', 'на', 'с', 'по', 'не', 'от',
  'за', 'а', 'у', 'о', 'из', 'до', 'при', 'про', 'к', 'ну', 'вот', 'так', 'более', 'менее', 'только',
  'чтобы', 'можно', 'свою', 'свои', 'свой', 'его', 'её', 'ее', 'них', 'их', 'кто', 'который', 'которая',
  'очень', 'хороший', 'хорошая', 'хорошее', 'новый', 'новая', 'новое', 'также', 'уже', 'быть',
]);
...
export async function embedMany(texts) {
  ...
  try {
    ...
    for (const t of missing) {
      ...
      if (Array.isArray(v)) { embedMem.set(t, v); out.set(t, v); } else still.push(t);
    }
    ...
    for (const [t, v] of pairs) { embedMem.set(t, v); out.set(t, v); }
    try {
...
// Real matching: semantic similarity between the user's lots and other lots,
// blended with price proximity. Without own lots — ranks by cross-demand
// (how many other users' "wants" match this lot).
export async function computeMatches({ myLots = [], others = [] }) {
  ...
  if (myLots.length) {
    ...
    matches = others.map(o => {
      const myOffer = offerText(my), myDemand = demandText(my);
      const theirOffer = offerText(o), theirDemand = demandText(o);
      const s1 = sim(vectors, theirDemand, myOffer); // они хотят то, что я отдаю
      const s2 = sim(vectors, theirOffer, myDemand); // я хочу то, что они отдают
      const s = Math.max(s1, s2);
      const denom = Math.max(o.value, my.value, 1);
      const priceScore = denom > 0 ? Math.max(0, 1 - Math.abs(o.value - my.value) / denom) : 0;
      const score = Math.max(0, Math.min(99, Math.round(100 * (0.7 * s + 0.3 * priceScore))));
      const owner = o.owner?.name || 'партнёр';
      const why = s1 >= s2
        ? `${owner} ищет «${(my.title || '').split(',')[0]}» — а вы как раз это отдаёте`
        : `Вы ищете «${(my.wants || '').split(',')[0]}», а ${owner} отдаёт «${(o.title || '').split(',')[0]}»`;
      return {
        lot: o,
        score,
        why,
    ...
    matches = others.map(o => {
      const offers = others.filter(x => x.id !== o.id);
      const scores = offers.map(x => sim(vectors, offerText(o), demandText(x)));
      const best = Math.max(0, ...scores);
      const count = scores.filter(s => s > 0.25).length;
      const bestIdx = scores.indexOf(best);
      const topX = offers[bestIdx];
      const popularity = Math.min(0.25, ((o.views || 0) / 600) * 0.25 + (o.hot ? 0.06 : 0));
      const score = Math.max(0, Math.min(99, Math.round(100 * Math.min(1, best * 1.35 + popularity))));
      const why = topX
        ? `${topX.owner?.name || 'Кто-то'} хочет получить «${(o.title || '').split(',')[0]}» в обмен`
