...
// Эмбеддинги считаются в фоне, после отрисовки ленты — пользователь их не ждёт,
// поэтому таймаут щедрый. Короткий (4 с) здесь уже стоял и оказался вреднее
// медленного ответа: провайдер отвечает за ~8 с, вызов отваливался КАЖДЫЙ раз,
// кэш векторов не наполнялся, и мэтчинг постоянно работал на запасном
// пересечении слов вместо семантики.
...
function fallbackDraft(input) {
  ...
  const condition = isService ? 'Услуга' : 'Хорошее';
...
Верни ТОЛЬКО JSON без markdown и без пояснений по схеме:
{
  ...
  "condition": "Новое|Отличное|Хорошее|Рабочее, со следами|Услуга",
...
- Для cat=service оценивай ОДНУ работу или ОДНУ смену аренды, а не стоимость самой техники и не месячный доход. «Аренда экскаватора» — это цена смены (несколько тысяч баллов), а не цена экскаватора.
...
function sanitizeDraft(d, input, fallback) {
  ...
  const condition = ['Новое', 'Отличное', 'Хорошее', 'Рабочее, со следами', 'Услуга'].includes(d.condition) ? d.condition : fallback.condition;
...
// Косинус на многоязычной модели не опускается к нулю: у двух совершенно
// несвязанных русских фраз («ноутбук» / «кролики») он ~0.47, у синонимов
// («помощь с уборкой» / «клининг») ~0.81. Если брать сырое значение за
// «долю совпадения», несвязанные вещи получают 60-70% — что и происходило.
// Растягиваем реальный коридор в 0..1.
const SIM_FLOOR = 0.46;
const SIM_CEIL = 0.82;
const calibrate = (s) => Math.max(0, Math.min(1, (s - SIM_FLOOR) / (SIM_CEIL - SIM_FLOOR)));

function sim(vectors, a, b) {
  const va = vectors.get(a), vb = vectors.get(b);
  // tokenSim уже нормирован на 0..1 и калибровки не требует
  if (va && vb) return calibrate(cosine(va, vb));
  return tokenSim(a, b);
}

// Индекс похожести для внешних потребителей (движок цепочек). Один раз
// считает векторы для всего корпуса и отдаёт готовую функцию сравнения —
// иначе граф на 400 лотах заказывал бы эмбеддинги на каждое ребро.
export async function similarityIndex(texts) {
  const vectors = await embedMany(texts);
  return {
    sim: (a, b) => sim(vectors, a, b),
    // без ключа/при сбое провайдера падаем на пересечение слов — знать об
    // этом полезно: пороги для токенов и для эмбеддингов разные
    semantic: vectors.size > 0,
  };
}

export const lotOfferText = offerText;
export const lotDemandText = demandText;

// Ниже этого порога подборка вреднее пустоты: лучше показать два честных
// совпадения, чем четыре случайных.
...
export async function computeMatches({ myLots = [], others = [], myWants = '' }) {
  ...
  // Учитываются ВСЕ мои лоты, а не первый: раньше бралось myLots[0], и у
  // человека с десятком объявлений мэтчинг смотрел только на одно.
  const myOffers = myLots.map(l => ({ lot: l, text: offerText(l) })).filter(o => o.text);
  // Спрос — вишлист профиля плюс «на что меняю» у каждого моего лота.
  const myDemands = [(myWants || '').trim(), ...myLots.map(demandText)].filter(Boolean);

  // Для доплаты нужен конкретный мой лот. Если владельца заинтересовало
  // что-то моё — берём именно его, иначе ближайший по цене.
  const nearestByValue = (target) => myLots.reduce(
    (best, l) => (best === null || Math.abs(l.value - target) < Math.abs(best.value - target) ? l : best),
    null,
  );

  let matches;

  if (myOffers.length || myDemands.length) {
    matches = others.map(o => {
      const oDemand = demandText(o);
      const oOffer = offerText(o);

      // они хотят то, что отдаю я — по лучшему из моих лотов
      let s1 = 0, bestMine = null;
      for (const m of myOffers) {
        const v = sim(vectors, oDemand, m.text);
        if (v > s1) { s1 = v; bestMine = m.lot; }
      }
      // я хочу то, что отдают они — по лучшей из моих хотелок
      let s2 = 0, bestDemand = '';
      for (const d of myDemands) {
        const v = sim(vectors, oOffer, d);
        if (v > s2) { s2 = v; bestDemand = d; }
      }

      const s = Math.max(s1, s2);
      const counterpart = bestMine || nearestByValue(o.value);
      const myValue = counterpart ? counterpart.value : 0;
      const denom = Math.max(o.value, myValue, 1);
      const priceScore = Math.max(0, 1 - Math.abs(o.value - myValue) / denom);
      // цена лишь уточняет: её прежний вес 0.3 давал 30% совпадения
      // любому нерелевантному лоту с похожей ценой
      const score = clampScore(100 * (0.85 * s + 0.15 * priceScore));
      const owner = o.owner?.name || 'партнёр';
      const why = (s2 >= s1 || !bestMine)
        ? `Вы ищете «${bestWant(bestDemand, o)}», а ${owner} отдаёт «${first(o.title)}»`
        : `${owner} ищет «${first(bestMine.title)}» — а вы как раз это отдаёте`;
      return {
        lot: o,
        score,
        why,
        // какой именно мой лот предлагается в обмен — раньше всегда первый
        myLotId: counterpart ? counterpart.id : null,
        topup: Math.abs(o.value - myValue),
        dir: o.value > myValue ? 'you-pay' : 'they-pay',
      };
    });
  } else if (myWants.trim()) {
    // нет своих лотов, но есть вишлист — «вам подойдёт»
    matches = others.map(o => {
      const s = sim(vectors, offerText(o), myWants.trim());
      const popularity = Math.min(0.12, (o.views || 0) / 600 * 0.12 + (o.hot ? 0.04 : 0));
      const score = clampScore(100 * Math.min(1, s * 1.45 + popularity));
      const owner = o.owner?.name || 'партнёр';
      const why = `Вы ищете «${bestWant(myWants, o)}» — ${owner} отдаёт «${first(o.title)}»`;
